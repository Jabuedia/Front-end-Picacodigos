...
import AppVue from '../App.vue';
    ...
    export default({
        ...
        async consultarApartamentos() {
            ...
            fetch('http://132.145.156.154:8080/PicacodigoAdmin/api/facturas', options)
                .then(async (response) => {
                    if (!response.ok) {
            ...
            console.log(idinmu);
            const direccion = 'http://132.145.156.154:8080/PicacodigoAdmin/api/inmuebles';
            const options = {
                method: 'GET',
        ...
        async actualizarPersona (fechaFactura,mesFactura,totalFactura,objetoIn, idpers){
            const direccion = 'http://132.145.156.154:8080/PicacodigoAdmin/api/facturas/'+idpers;
            const options = {
            method: 'PUT',
            body: JSON.stringify(
                {
